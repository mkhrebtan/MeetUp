import { AfterViewInit, Component, inject, OnDestroy, OnInit, output, signal } from '@angular/core';
...
@Component({
        ...
        <div class="flex flex-col gap-4">
          <div class="flex flex-col items-center gap-2">
            <p-select
              ...
              (onChange)="changeAudioInput($event.value)"
              optionLabel="label"
          ...
          <div class="flex flex-col items-center gap-2">
            <p-select
              ...
              (onChange)="changeAudioOutput($event.value)"
              optionLabel="label"
              optionValue="deviceId"
              size="small"
              fluid
          ...
          <div class="flex flex-col items-center gap-2">
            <p-select
              ...
              [(ngModel)]="selectedVideoInputId"
              (onChange)="changeVideoInput($event.value)"
              optionLabel="label"
              optionValue="deviceId"
              size="small"
              fluid
  ...
  `,
  styles: [],
  imports: [Button, Select, ReactiveFormsModule, FormsModule, ParticipantVideoComponent],
})
export class MeetingStageComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  async ngOnInit() {
    ...
    this.selectedAudioInputId =
      this.devices.audioInputs.find((i) => i.deviceId === 'default')?.deviceId ??
      this.devices.audioInputs[0].deviceId;
    this.selectedVideoInputId =
      this.devices.videoInputs.find((i) => i.deviceId === 'default')?.deviceId ??
      this.devices.videoInputs[0].deviceId;
    this.selectedAudioOutputId =
      this.devices.audioOutputs.find((i) => i.deviceId === 'default')?.deviceId ??
      this.devices.audioOutputs[0].deviceId;
  }
