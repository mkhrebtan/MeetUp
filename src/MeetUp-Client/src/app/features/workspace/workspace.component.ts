...
export class WorkspaceComponent {
  ...
  joinWorkspaceForm = this.fb.group({
    inviteCode: ['', [Validators.required, Validators.minLength(12), Validators.maxLength(12), Validators.pattern(/^[A-Z0-9]{3}-[A-Z0-9]{3}-[A-Z0-9]{3}$/)]]
  });
  ...
  onCreateWorkspaceSubmit() {
    if (this.createWorkspaceForm.valid) {
      this.store.dispatch(WorkspaceActions.createWorkspace({name: this.createWorkspaceForm.value.name!}));
    }
  }

  onJoinWorkspaceSubmit() {
    if (this.joinWorkspaceForm.valid) {
      this.store.dispatch(WorkspaceActions.joinWorkspace({inviteCode: this.joinWorkspaceForm.value.inviteCode!}));
    }
  }
}
